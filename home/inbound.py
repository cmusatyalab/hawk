...
class InboundProcess:
            ...
            while not stop_event.is_set():
                ...
                if self._token:
                    ...
                    if self._sample_count == self.total_init_samples:
                        ...
                        else: pri_q = self._global_priority_queue
                        token_th = threading.Thread(target=self.token_thread, args=(pri_q,result_q, self._rotation_mode, data_name, stop_event))
                        token_th.start()
    ...
    def write_push(self, result_queue, temp_meta_data, data_name, data, local_counter):
                ...
                logger.info(f"Received {temp_meta_data['objectId']} {temp_meta_data['scoutIndex']} {temp_meta_data['score']}")
