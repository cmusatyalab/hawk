...
class FewShotModel(nn.Module):
    def __init__(self, args):
        ...
        if args.backbone_class == 'ConvNet':
            from hawk.trainer.few_shot.networks.convnet import ConvNet
            self.encoder = ConvNet()
        elif args.backbone_class == 'Res12':
            hdim = 640
            from hawk.trainer.few_shot.networks.res12 import ResNet
            self.encoder = ResNet()
        elif args.backbone_class == 'Res18':
            hdim = 512
            from hawk.trainer.few_shot.networks.res18 import ResNet
            self.encoder = ResNet()
        elif args.backbone_class == 'WRN':
            hdim = 640
            from hawk.trainer.few_shot.networks.WRN28 import Wide_ResNet
