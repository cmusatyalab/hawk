...
def drawBoxOrEllipse(inputs, class_name, axe, color, x_shape=0, mode="box", \
    ...
    if mode == "box":
        for box in inputs:
            # y1, y2, x1, x2 = box
            x_c, y_c, w, h = box
            #print("Box data:", x_c, y_c, w, h)
            
            y1, y2, x1, x2 = int(int(y_c)-int(h)/2), int(int(y_c)+int(h)/2), int(int(x_c)-int(w)/2), int(int(x_c)+int(w)/2)
            #print("box coords:", x1, y1, x2, y2)
            if x1 < 0:
                #x1 += x_shape
            ...
            if int(x_c) < 15: x_c = 15
            axe.text(int(x_c), y1-3, class_name, size=6, verticalalignment=verticalalignment,horizontalalignment=horizontalalignment,
...
def drawRadarBoxes(stereo_left_image, RD_img, radar_instances, all_classes, colors, axes):
    """ draw only boxes on the input images """
    #assert len(radar_instances["boxes"]) == len(radar_instances["classes"])
    #assert len(axes) == 1
    #print("RD img shape:", RD_img.shape)
    bbox2d = radar_instances.split()[1:]
    cls = radar_instances.split()[0]
    #print("Class:", cls)
    color = colors[int(cls)]
    ### draw box
    mode = "box" # either "box" or "ellipse"
    ### boxes information added in the ground truth dictionary
    RD_box = np.array([[bbox2d[0], bbox2d[1], bbox2d[2], bbox2d[3]]])
    #print("RD box:", RD_box, cls)
    class_labels = {'0':"person", '1':"bicycle", '2':"car", '3':"motorcycle", '4':"bus", '5':"truck"}
    ### draw boxes
    drawBoxOrEllipse(RD_box, class_labels[cls], axes[1], color, \
                    x_shape=RD_img.shape[1], mode=mode)
