...
def calculate_performance(y_true, y_pred):
    ...
    print("Model Classification GT:{} Pred:{} AUC {} ".format(sum(y_true), sum(y_pred_binary), ap))
    print("Confusion matrix \n {}\n".format(confusion_matrix(y_true, y_pred_binary)))
    return ap
...
def run(data,
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(tqdm(dataloader, desc=s)):
        ...
        for si, pred in enumerate(out):
            ...
            if nl:
                ground_truth_pred.append(1)
            else:
                ground_truth_pred.append(0)

            if len(pred) == 0:
                classification_pred.append(0)
                if nl:
                    stats.append((torch.zeros(0, niou, dtype=torch.bool), torch.Tensor(), torch.Tensor(), tcls))
                continue
            else:
                classification_pred.append(pred[0, 4].cpu())
    ...
    if len(stats) and stats[0].any():
        auc = calculate_performance(ground_truth_pred, classification_pred)
    ...
    for i, c in enumerate(ap_class):
        maps[c] = ap[i]
    return auc, (mp, mr, map50, map, *(loss.cpu() / len(dataloader)).tolist()), maps, t
