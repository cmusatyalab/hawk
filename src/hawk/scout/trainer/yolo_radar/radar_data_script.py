...
def visualize(RAD, RD_path):
    ##This function is only used for generating new heat maps if wanting to visualize the new created samples.
    RD = np.load(RD_path)
    RD = np.sum(RD, axis=2)
    RD = np.transpose(RD)
    ...
    drawer.drawRadarBoxes(stereo_left_image, RD_img, gt_instances, classes, colors, axes)   
...
for label_file in tqdm(sorted(label_files)):
    ...
    image_name = os.path.join(ORIGINAL_IMAGES_DIR, part + "/" + label_base.split('.')[0] + ".npy")
